...
Client::Client() {
  ...
  m_me.m_name = input;
  std::cout << "Hi " << input << " we are now connecting you to the server!"
            << std::endl;
}
...
void Client::askForId() {
  std::cout << "askForId()\n";
  m_socket.async_send(
      boost::asio::buffer("i?" + m_me.m_name),
      boost::bind(&Client::handleErrors, this, boost::asio::placeholders::error,
                  boost::asio::placeholders::bytes_transferred));
  m_socket.async_receive(
      boost::asio::buffer(m_buffer),
      boost::bind(&Client::parseId, this, boost::asio::placeholders::error,
                  boost::asio::placeholders::bytes_transferred));
}
...
void Client::startReceiving() {
  m_socket.async_receive(
      boost::asio::buffer(m_buffer),
      boost::bind(&Client::handleData, this, boost::asio::placeholders::error,
                  boost::asio::placeholders::bytes_transferred));
}
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    case state::init: {
      if (std::isdigit(input[i])) {
        ...
        std::cerr << "message was: " << input << " i = " << i
                  << " char: " << input[i] << std::endl;
        return;
      }
      break;
    }
    ...
    case state::end2: {
      falseTeam = static_cast<unsigned>(
          std::stoul(input.substr(index_start, i - index_start)));
...
void Client::parseId(ErrorCode &err, size_t trans) {
  if (!err) {
    ...
    std::cout << "received id: " << m_me.m_id << "team: " << m_me.m_team
              << std::endl;
    hasId = true;
  } else {
    std::cerr << "NetworkClient::handle_receive:" << err.message();
...
void Client::handleErrors(ErrorCode &error, std::size_t bytes_transferred) {
  if (error) {
    std::cerr << "Client error: " << error.message()
              << "bytes transferred: " << bytes_transferred << ", exiting"
              << std::endl;
    std::exit(1);
  }
}

void Client::disconnect() { send("end"); }

const std::vector<Player> &Client::getPlayers() { return m_players; }

const Player &Client::getMe() const { return m_me; }

Message<Transform> &Client::getMessage() { return m_lastMessage; }

bool Client::hasStarted() const { return m_gameStarted.load(); }

const Score &Client::getScore() const { return m_score; }

bool Client::hasEnded() const { return m_gameEnded.load(); }

bool Client::hasStateChanged() const { return m_stateChanged.load(); }

const GameStateChange &Client::getState() const { return m_state; }

void Client::resetStateChanged() { m_stateChanged.store(false); }
