...
void Server::listen() {
  m_socket.async_receive_from(
      boost::asio::buffer(m_buffer), m_pending,
      boost::bind(&Server::handleRequest, this,
...
void Server::sendData(const std::vector<NetworkId> &ids,
                      const std::vector<Transform> &transforms) {
  std::string message{};
  for (size_t i = 0; i < ids.size(); ++i) {
    const auto &position = transforms[i].m_position;
    message += std::to_string(ids[i].id) + "_" + std::to_string(position.m_x) +
               ";" + std::to_string(position.m_y) + ",";
  }
  respondAll(message);
}
...
void Server::respond(const udp::endpoint &cl, const std::string &response) {
  m_socket.async_send_to(
      boost::asio::buffer(response), cl,
      boost::bind(&Server::handleErrors, this, boost::asio::placeholders::error,
                  boost::asio::placeholders::bytes_transferred));
}

void Server::init() { listen(); }

void Server::handleErrors(ErrorCode &err, size_t trans) {
  if (err) {
    std::cerr << "Client error: " << err.message()
              << "bytes transferred: " << trans << ", exiting" << std::endl;
    std::exit(1);
  }
}
...
void Server::emplaceClient(udp::endpoint endpoint, size_t trans) {
  ...
  for (unsigned short i = 0; i < ServerGameConstants::kMaxNumberOfPlayers;
       ++i) {
    if (!m_clients[i] &&
        viable_index == ServerGameConstants::kMaxNumberOfPlayers - 1)
      viable_index = i;
  ...
  m_clients[viable_index] = std::make_unique<Client>(
      std::move(endpoint), std::move(newName), viable_index + 1);
  m_clients[viable_index]->baseInfo.m_team =
      m_clientNr >= ServerGameConstants::kMaxNumberOfPlayers / 2;

  // test
  std::cout << "new guy's name: " << m_clients[viable_index]->baseInfo.m_name
            << " id: " << m_clients[viable_index]->baseInfo.m_id << std::endl;
  ...
  respond(m_clients[viable_index]->m_endpoint,
          {std::to_string(viable_index + 1) + ":" + std::to_string(team)});
  if (m_clientNr == ServerGameConstants::kMaxNumberOfPlayers) {
    std::cout << "starting game" << std::endl;
    std::string startMessage("s;");
    for (auto &client : m_clients) {
      startMessage += client
                          ? std::to_string(client->baseInfo.m_id) + "_" +
                                client->baseInfo.m_name + "_" +
                                (client->baseInfo.m_team ? "1" : "0") + "."
                          : ""; // <<< should not happen
    }
...
void Server::parseInput(const std::string &message, size_t length) {
  enum class state { init, new_pl, ind, discnct_1, discnct_2 };
    ...
    case state::ind: {
      ...
      if (message[i] == '_') {
        index = static_cast<size_t>(
            std::stol(message.substr(index_start, i - index_start)));
        if (!m_clients[index - 1] ||
            !endpointEq(m_pending, m_clients[index - 1]->m_endpoint))
          return;
...
Message<MovementInputHolder> &Server::getMessage() { return m_message; }

bool Server::hasStarted() const { return m_gameStarted.load(); }
