...
inline void applyInputForce(RigidBody &body,
                            const MovementInputHolder &inputHolder,
                            float coef) {
...
template <typename TSettings> class ServerNetworkReceiverSystem {
...
private:
  ...
  void updateInputs(std::vector<NetworkId> ids,
                    std::vector<MovementInputHolder> inputs) {
    m_componentManager
        ->template forEntitiesMatching_S<SystemSignature_Network_Rigid>(
            [&ids, &inputs](NetworkId *id, RigidBody *body) {
              for (int i = 0; i < ids.size(); ++i) {
                if (ids[i].id == id->id)
                  applyInputForce(
                      *body, inputs[i],
                      ServerGameConstants::kInputMovementCoefficient);
              }
            });
  }
  ...
  void gameStarted() {
    ...
    std::cout << "first player: " << players[0].m_name
              << " id: " << players[0].m_id << std::endl;
    std::cout << "second player: " << players[1].m_name
              << " id: " << players[1].m_id << std::endl;
    Id i = 0;
    std::lock_guard<std::mutex> lock(m_componentManager->componentsMutex);
    m_componentManager
        ->template forEntitiesMatching<SystemSignature_Network_Player>(
            [&players, &i](NetworkId *id, Transform *transform,
                           PlayerComp *playerComp) {
              id->id = players[i].m_id;
              transform->m_position =
                  ServerGameConstants::kStartingPositions[i];
              ++i;
            });

    m_componentManager->template forEntitiesMatching<SystemSignature_Ball>(
        [](Transform *transform, BallComp *ballComp) {
  ...
  void run() {
    ...
    while (true) {
      ...
      bool accept;
      m_componentManager
          ->template forEntityMatching_S<SystemSignature_GameState>(
              m_GameStateId,
              [&accept](GameState *state) { accept = state->m_ReceiveInput; });
      if (!accept)
        continue;
...
public:
  ...
  void setServer(Server *server) { m_UdpServer = server; }

  void start() {
    m_GameStateId =
        m_componentManager
            ->template findEntityMatching<SystemSignature_GameState>();
