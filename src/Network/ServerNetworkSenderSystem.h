...
template <typename TSettings> class ServerNetworkSenderSystem {
...
private:
  ...
  void prepareData(std::vector<NetworkId> &ids,
...
public:
  ...
  void setServer(Server *server) { m_playerServer = server; }
  ...
  void run(float dt) {
    ...
    m_componentManager
        ->template forEntityMatching_S<SystemSignature_GameStateChange>(
            m_GameStateChangeId, [this](GameStateChange *state) {
              m_StateChange = *state;
    ...
    if (m_StateChange.m_GameOver || m_StateChange.m_Team1Scored ||
        m_StateChange.m_Team2Scored) {
