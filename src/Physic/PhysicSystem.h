...
inline void resolveCollision(const Collision &collision) {
  ...
  float r =
      std::min(collision.first.m_restitution, collision.second.m_restitution);
  ...
  // apply impulse, so add to velocities force relative to mass of the rigid
  // body
...
inline bool checkForCollisionCircle_Circle(const CircleCollider *shape1,
                                           const Transform *transform1,
                                           const CircleCollider *shape2,
  ...
  if (d != 0) {
    ...
    // now we have to calculate vector perpendicular to vector n and normalize
    // it
    ...
    // this should not be really happening, but just in case when distance is 0
...
inline bool checkForCollisionRectangle_Rectangle(
    const RectangleCollider *shape1, const Transform *transform1,
  ...
  // for rectangles this means that we only solve collision in one axis, the one
  // that has greater overlap
...
inline bool checkForCollisionRectangle_Circle(const RectangleCollider *shape1,
                                              const Transform *transform1,
                                              const CircleCollider *shape2,
                                              const Transform *transform2,
                                              Collision *collision,
  ...
  if (inside) {
    ...
    else if (normal.m_y == closest.m_y)
...
inline void processTriggerCollisionRectangle_Circle(
    const RectangleCollider *shape1, const Transform *transform1,
    ColliderTrigger *trigger, const CircleCollider *shape2,
    const Transform *transform2) {
  if (!ServerGameConstants::kCollisionMatrix[shape1->m_CollisionLayer]
                                            [shape2->m_CollisionLayer])
    return;
  // std::cout << "checking trigger collision rectangle circle" << std::endl;
  if (!checkForCollisionRectangle_Circle(shape1, transform1, shape2, transform2,
                                         nullptr, true))
    return;
  trigger->m_Triggered = true;
}

inline void processTriggerCollisionRectangle_Rectangle(
    const RectangleCollider *shape1, const Transform *transform1,
    ColliderTrigger *trigger, const RectangleCollider *shape2,
    const Transform *transform2) {
  if (!ServerGameConstants::kCollisionMatrix[shape1->m_CollisionLayer]
                                            [shape2->m_CollisionLayer])
    return;
...
inline void processTriggerCollisionCircle_Circle(const CircleCollider *shape1,
                                                 const Transform *transform1,
  ...
  if (!ServerGameConstants::kCollisionMatrix[shape1->m_CollisionLayer]
                                            [shape2->m_CollisionLayer])
    return;
  if (!checkForCollisionCircle_Circle(shape1, transform1, shape2, transform2,
                                      nullptr, true))
    return;
  trigger->m_Triggered = true;
}

inline void processCollisionRectangle_Circle(const RectangleCollider *shape1,
                                             RigidBody *body1,
                                             const Transform *transform1,
                                             const CircleCollider *shape2,
                                             RigidBody *body2,
...
inline void processCollisionRectangle_Rectangle(const RectangleCollider *shape1,
                                                RigidBody *body1,
                                                const Transform *transform1,
                                                const RectangleCollider *shape2,
                                                RigidBody *body2,
                                                const Transform *transform2) {
  if (!ServerGameConstants::kCollisionMatrix[shape1->m_CollisionLayer]
                                            [shape2->m_CollisionLayer])
...
inline void processCollisionCircle_Circle(const CircleCollider *shape1,
                                          RigidBody *body1,
                                          const Transform *transform1,
                                          const CircleCollider *shape2,
                                          RigidBody *body2,
...
template <typename TSettings> class PhysicSystem {
private:
...
public:
  ...
  // only if you wish to run physics as internal system, on the same thread as
  // engine
  void run(float dt) { runPhysicUpdate(dt); }

  void runPhysicUpdate(float dt) {

    std::lock_guard<std::mutex> lock(m_ComponentManager->componentsMutex);

    m_ComponentManager->template forEntitiesMatching<SystemSignature_Movable>(
        [dt, this](RigidBody *rigidBody, Transform *transform) {
    ...
    m_ComponentManager->template forEntitiesMatchingPairs<
        SystemSignature_Circle_Collider_Body,
        SystemSignature_Circle_Collider_Body>(processCollisionCircle_Circle);
    m_ComponentManager->template forEntitiesMatchingPairs<
        SystemSignature_Rectangle_Collider_Body,
        SystemSignature_Rectangle_Collider_Body>(
        processCollisionRectangle_Rectangle);
    m_ComponentManager->template forEntitiesMatchingPairs<
        SystemSignature_Rectangle_Collider_Body,
        SystemSignature_Circle_Collider_Body>(processCollisionRectangle_Circle);
