...
template <typename TSettings> class ServerLogicSystem {
  ...
  void resetPositions() {
    ...
    int i = 0;
    m_manager->template forEntitiesMatching<SystemSignature_Player_Rigid>(
        [&i](RigidBody *body, Transform *transform, PlayerComp *playerComp) {
    ...
    m_manager->template forEntitiesMatching<SystemSignature_Ball_Rigid>(
        [](RigidBody *body, Transform *transform, BallComp *ballComp) {
...
public:
  void setManager(ComponentManager<TSettings> *manager) { m_manager = manager; }

  void start() {
    m_LeftGoalId =
        m_manager->findGameObjectByTag(ServerGameConstants::kLeftGoalTag);
    m_RightGoalId =
        m_manager->findGameObjectByTag(ServerGameConstants::kRightGoalTag);
    m_stateHolderId =
        m_manager->template findEntityMatching<SystemSignature_GameState>();
    m_GameStateChangeId =
        m_manager
  ...
  void run(float dt) {
    ...
    if (m_time > ServerGameConstants::kGameTime) {
      ...
      m_manager->template forEntityMatching<SystemSignature_GameState>(
          m_stateHolderId,
          [](GameState *state) { state->m_ReceiveInput = false; });

      m_manager->template forEntityMatching<SystemSignature_GameStateChange>(
          m_GameStateChangeId,
          [](GameStateChange *change) { change->m_GameOver = true; });
      // now we may want to call quit after we send game over message to other
      // players
    ...
    if (m_WaitingToResumeGame) {
      ...
      if (m_WaitingTime > ServerGameConstants::kWaitingToResumeGameTime) {
        ...
        m_manager->template forEntityMatching<SystemSignature_GameState>(
            m_stateHolderId,
            [](GameState *state) { state->m_ReceiveInput = true; });
        // here we set receive input to true
        m_WaitingTime = 0;
        m_WaitingToResumeGame = false;

        m_manager->template forEntityMatching<SystemSignature_ColliderTrigger>(
            m_LeftGoalId,
            [this](ColliderTrigger *trigger) { trigger->m_Triggered = false; });
    ...
    m_manager->template forEntityMatching<SystemSignature_ColliderTrigger>(
        m_LeftGoalId, [this](ColliderTrigger *trigger) {
          if (!trigger->m_Triggered)
            return;
          std::cout << "left goal scored" << std::endl;
          trigger->m_Triggered = false;
          m_manager
              ->template forEntityMatching<SystemSignature_GameStateChange>(
                  m_GameStateChangeId, [](GameStateChange *change) {
    ...
    m_manager->template forEntityMatching<SystemSignature_ColliderTrigger>(
        m_RightGoalId, [this](ColliderTrigger *trigger) {
          if (!trigger->m_Triggered)
            return;
          trigger->m_Triggered = false;
          m_manager
