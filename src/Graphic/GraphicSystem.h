...
template <typename TSettings> class GraphicSystem {
  ...
  void renderRectangles(int layer) {
    m_componentManager->template forEntitiesMatching<
        SystemSignature_RectangleGraphic>([this, layer](
        const RectangleShape *shape, const Transform *transform) {
      SDL_SetRenderDrawColor(this->getRenderer(), shape->m_color.r,
                             shape->m_color.g, shape->m_color.b,
                             shape->m_color.a);
      ...
      if (!shape->m_enabled)
        return;
      SDL_Rect fillRect = {
          static_cast<int>(std::round(transform->m_position.m_x + shape->x)),
          static_cast<int>(std::round(transform->m_position.m_y + shape->y)),
          shape->mWidth, shape->mHeight};
  ...
  void renderSprites(int layer) {
    m_componentManager
        ->template forEntitiesMatching<SystemSignature_SpriteGraphic>(
            [this, layer](Sprite *sprite, const Transform *transform) {
              ...
              if (!sprite->m_Texture) {
                ...
                sprite->m_Texture = TextureSmartPtr(
                    SDL_CreateTextureFromSurface(this->getRenderer(), surface));
                if (!sprite->m_Texture) {
                  std::cerr
                      << "Unable to create texture, error: " << SDL_GetError()
                      << std::endl;
              ...
              SDL_Rect renderQuad = {
                  static_cast<int>(std::round(sprite->m_positionOffset.m_x +
                                              transform->m_position.m_x)),
                  static_cast<int>(std::round(sprite->m_positionOffset.m_y +
                                              transform->m_position.m_y)),
                  sprite->m_Width, sprite->m_Height};
              SDL_RenderCopy(this->getRenderer(), sprite->m_Texture.get(),
                             nullptr, &renderQuad);
  ...
  void renderLabels(int layer) {
    m_componentManager
        ->template forEntitiesMatching<SystemSignature_LabelGraphic>(
            [this, layer](Label *label, const Transform *transform) {
              ...
              if (!label->m_Texture) {
                ...
                SurfaceSmartPtr textSurface =
                    SurfaceSmartPtr(TTF_RenderText_Solid(
                        font, label->m_text.c_str(), label->m_fontColor));
                if (textSurface == nullptr) {
                  std::cerr << "Unable to create surface from font! error: "
                            << TTF_GetError() << std::endl;
                }

                label->m_Texture = TextureSmartPtr(SDL_CreateTextureFromSurface(
                    getRenderer(), textSurface.get()));
                label->m_Width = textSurface->w;
                label->m_Height = textSurface->h;
              }

              SDL_Rect renderQuad = {
                  static_cast<int>(std::round(label->m_positionOffset.m_x +
                                              transform->m_position.m_x)),
                  static_cast<int>(std::round(label->m_positionOffset.m_y +
                                              transform->m_position.m_y)),
                  label->m_Width, label->m_Height};
              SDL_RenderCopy(this->getRenderer(), label->m_Texture.get(),
                             nullptr, &renderQuad);
...
public:
  ...
  void draw() {
    ...
    {
      // this ensures that we draw all objects without being interrupted by
      // other threads
      std::lock_guard<std::mutex> lock(m_componentManager->componentsMutex);
      for (int layer = 0; layer < ClientGameConstants::kNumberOfLayers;
           ++layer) {
  ...
  GraphicSystem()
      : m_componentManager(nullptr),
        m_ScreenHeight(ClientGameConstants::kWindowHeight),
        m_ScreenWidth(ClientGameConstants::kWindowWidth),
        m_WindowHolder("MainWindow", SDL_WINDOWPOS_UNDEFINED,
                       SDL_WINDOWPOS_UNDEFINED, m_ScreenWidth, m_ScreenHeight,
                       SDL_WINDOW_SHOWN),
        m_RendererHolder(m_WindowHolder.window, -1,
  ...
  SDL_Surface *getSurface(std::string path) {
    ...
    if (result == m_SurfaceHolder.surfaces.end()) {
      ...
      if (loadedSurface == nullptr) {
        std::cout << "unable to load surface at path: " << path
                  << ", error: " << IMG_GetError() << std::endl;
        return nullptr;
      }
      // is save is on, it means we have to cache the surface for future use, so
      // we create
      // a new smart pointer to surface and emplacfe it into the map for
      // surfaces
  ...
  TTF_Font *getFont(std::string path, int size) {
    ...
    if (result1 == m_FontHolder.fonts.end()) {
      ...
      if (font == nullptr) {
        std::cout << "unable to load font at path: " << path
                  << ", error: " << TTF_GetError() << std::endl;
        return nullptr;
      }
      FontSizeMap map;
      map.emplace(size, FontSmartPtr(font));
      // create a new map with the newly created surface and the emplace the map
      // into fonts
  ...
  SDL_Renderer *getRenderer() { return m_RendererHolder.renderer; }
