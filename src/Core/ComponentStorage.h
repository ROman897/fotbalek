...
template <typename TSettings> class ComponentStorage {
private:
  ...
  using UnpackedTuple = typename decltype(hana::unpack(
      componentSettings::componentList, hana::template_<TupleOfVectors>))::type;
  UnpackedTuple m_components;

public:
  void grow(std::size_t mNewCapacity) {

    hana::for_each(m_components,
                   [mNewCapacity](auto &v) { v.resize(mNewCapacity); });
  }

  template <typename T> constexpr auto &getComponentVector() {
  ...
  template <typename T> constexpr auto getComponentId() {
